...
inline unsigned long long fio_crc64(const unsigned char *buffer, unsigned long length)
{
...
int main(){
    //generate test cases
    std::ofstream genOutput;
    genOutput.open("test_input.txt");
    ...
    if (inputFile.is_open()) {
        ...
        while (std::getline(inputFile, line)) {
           outputFile << consistentHashForString(line, 2147483647) << std::endl;
        }
        inputFile.close();
    }
