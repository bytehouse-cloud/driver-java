...
public class GatewayConnectionITest extends AbstractByteHouseITest {
    ...
    public void createDatabase_success() throws SQLException {
        Connection connection = getConnection();
        Statement stmt = connection.createStatement();
        assertDoesNotThrow(() -> stmt.execute("CREATE DATABASE jdbc_itest_db"));
        assertDoesNotThrow(() -> stmt.execute("DROP DATABASE jdbc_itest_db"));
    }
    ...
    public void createTable_success() throws SQLException {
        ...
        assertDoesNotThrow(() -> {
            stmt.execute(
                    "CREATE TABLE jdbc_itest_db.npc_cases\n" +
                            "(" +
                            "    year Int16," +
                            "    npc String," +
                            "    offence String," +
    ...
    public void insert_success() throws SQLException {
        ...
        int numUpdated = stmt.executeUpdate(
                "INSERT INTO jdbc_itest_db.npc_cases VALUES " +
                        "(2011,'Central firemen Division - Total','Unlicensed Moneylending',242)"
        );
        assertEquals(1, numUpdated);

        ResultSet rs = stmt.executeQuery("SELECT * FROM jdbc_itest_db.npc_cases");
        assertTrue(rs.next());
